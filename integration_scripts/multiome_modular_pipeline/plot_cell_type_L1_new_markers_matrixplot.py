#!/usr/bin/env python3
"""
Script to generate marker gene expression matrixplot for cell_type_L1 annotations
Creates heatmap-style matrix plots showing mean expression across cell types

This script:
1. Loads annotation_final.h5ad
2. Loads biomarker genes from DEGs_cell_type_L1FC_0_25/biomarkers/sig_deg_lists
...
BIOMARKER_BASE_DIR = '/beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/combine_data/results_from_raw/DEGs_cell_type_L1FC_0_25/biomarkers/sig_deg_lists'
OUTPUT_DIR = '/beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/ADDITIONAL_PLOTS/matrixplots/L1'
CELL_TYPE_COLUMN = 'cell_type_L1'
# Generate multiple plots with different numbers of top markers
TOP_N_GENES_LIST = [5, 10, 15]  # Will create separate plots for each

def load_cell_type_markers(biomarker_dir, cell_type, top_n=5):
    """Load top marker genes for a specific cell type"""
    folder_name = cell_type
...
def create_matrixplot(adata, marker_genes_dict, output_dir, top_n_genes, cell_type_column='cell_type_L1'):
    ...
    # Figure sizing - L1 has fewer cell types (9) so different sizing
    n_cell_types = len(cell_types_in_data)
    n_genes = len(available_genes)

    # More aggressive width scaling to prevent x-label overlap
    if n_genes <= 50:
        fig_width = max(18, n_genes * 0.35)
        fig_height = max(8, n_cell_types * 1.0)
        fontsize = 12
    elif n_genes <= 100:
        fig_width = max(22, n_genes * 0.4)
        fig_height = max(9, n_cell_types * 1.0)
        fontsize = 11
    elif n_genes <= 150:
        fig_width = max(30, n_genes * 0.4)
        fig_height = max(10, n_cell_types * 1.1)
        fontsize = 10
    elif n_genes <= 200:
        fig_width = max(40, n_genes * 0.4)
        fig_height = max(11, n_cell_types * 1.1)
        fontsize = 9
    else:
        fig_width = max(50, n_genes * 0.2)
        fig_height = max(12, n_cell_types * 1.2)
        fontsize = 8

    try:
        plt.rcParams['font.size'] = fontsize
        plt.rcParams['axes.labelsize'] = fontsize + 3
        ...
        if fig is not None and hasattr(fig, 'get_axes') and len(fig.get_axes()) > 0:
            fig.suptitle(f'Cell Type Markers (L1) - Matrixplot - Top {top_n_genes} per cell type',
                        fontsize=fontsize + 7, fontweight='bold')
            plt.tight_layout()
...
def main():
    ...
    print("="*80)
    print("CELL TYPE L1 MARKER GENE MATRIXPLOT GENERATION")
    print("="*80)

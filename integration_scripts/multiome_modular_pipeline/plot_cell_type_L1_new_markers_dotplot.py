...
OUTPUT_DIR = '/beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/ADDITIONAL_PLOTS/dotplots/L1'
...
def create_cell_type_dotplot(adata, marker_genes_dict, output_dir, top_n_genes, cell_type_column='cell_type_L2_new'):
    ...
    # For L1 with 9 cell types, we need less vertical space than L2
    # More horizontal space for genes
    if n_genes <= 150:
        # Top 15 markers: ~135 genes
        fig_width = max(35, n_genes * 0.70)
        fig_height = max(10, n_cell_types * 1.2)
        gene_fontsize = 18
        celltype_fontsize = 20
        dot_max = 0.7
        dot_min = 0.0
    elif n_genes <= 200:
        # Top 20 markers: ~180 genes
        fig_width = max(45, n_genes * 0.65)
        fig_height = max(12, n_cell_types * 1.3)
        gene_fontsize = 16
        celltype_fontsize = 19
        dot_max = 0.7
        dot_min = 0.0
    else:
        # Top 25 markers: ~225 genes
        fig_width = max(55, n_genes * 0.60)
        fig_height = max(14, n_cell_types * 1.4)
        gene_fontsize = 14
        celltype_fontsize = 18
        dot_max = 0.7
        dot_min = 0.0
    ...
    try:
        ...
        plt.rcParams['axes.labelsize'] = gene_fontsize + 6
        plt.rcParams['axes.titlesize'] = gene_fontsize + 8
        plt.rcParams['xtick.labelsize'] = gene_fontsize
        plt.rcParams['ytick.labelsize'] = celltype_fontsize
        plt.rcParams['legend.fontsize'] = gene_fontsize + 4
        plt.rcParams['legend.title_fontsize'] = gene_fontsize + 6
        ...
        if fig is not None:
            title_fontsize = max(20, gene_fontsize + 8)
            # Add title
            fig.suptitle(f'Cell Type Markers (L1) - Top {top_n_genes} per cell type',
...
def main():
    ...
    print("="*80)
    print("CELL TYPE L1 NEW MARKER GENE DOTPLOT GENERATION")
    print("="*80)
    ...
    # Check if cell_type_L1_new exists

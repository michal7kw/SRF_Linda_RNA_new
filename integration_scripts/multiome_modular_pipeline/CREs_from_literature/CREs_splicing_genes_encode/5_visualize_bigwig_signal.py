...
visualizations of ATAC-seq signal at ENCODE cCREs, with focus on Ctrl vs Mut comparisons.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Visualize BigWig signal at splicing gene ENCODE cCREs',
    ...
    print("="*80)
    print("VISUALIZE BIGWIG SIGNAL AT SPLICING GENE ENCODE cCREs")
    print("="*80)
    print()
    ...
    for i, row in bed_df.iterrows():
        ...
        if (i + 1) % 100 == 0:
            print(f"  Processing CRE {i+1}/{len(bed_df)}...")
        ...
        fc_check = fold_change if not np.isnan(fold_change) else 1.0
        info['is_significant'] = max_val >= args.min_signal and (fc_check >= args.min_fc or fc_check <= (1.0/args.min_fc))

    print(f"  Processed {len(bed_df)} CREs")
    ...
    if not args.skip_individual:
        ...
        sig_cres = [info for info in cre_info_nestin if info.get('is_significant', False)]
        print(f"  Significant CREs: {len(sig_cres)}")

        if args.parallel > 1:
            print(f"  Using {args.parallel} parallel processes...")
            plot_args = []
            for info in sig_cres:
                output_file = f"{output_dir}/individual_Nestin_{info['gene']}_{info['cre_id']}.png"
                args_tuple = (bw_nestin_ctrl, bw_nestin_mut, info['chrom'], info['start'], info['end'],
                             window, bins, info['gene'], info['cre_id'], 'Nestin', output_file,
                             args.individual_dpi)
                plot_args.append(args_tuple)

            with Pool(args.parallel) as pool:
                pool.map(_plot_single_cre_bigwig, plot_args)
            print(f"  Created {len(plot_args)} plots")
        else:
            for j, info in enumerate(sig_cres):
                output_file = f"{output_dir}/individual_Nestin_{info['gene']}_{info['cre_id']}.png"
                _plot_single_cre_bigwig((bw_nestin_ctrl, bw_nestin_mut, info['chrom'], info['start'], info['end'],
                                        window, bins, info['gene'], info['cre_id'], 'Nestin', output_file,
                                        args.individual_dpi))
                if (j + 1) % 20 == 0:
                    print(f"    Progress: {j+1}/{len(sig_cres)} plots")
    ...
    bw_emx1_ctrl = f"{bigwig_base}/GABA_Nestin-Ctrl.bw"  # Using Nestin-Ctrl as baseline
    ...
    if not args.skip_individual:
        ...
        sig_cres = [info for info in cre_info_emx1 if info.get('is_significant', False)]
        print(f"  Significant CREs: {len(sig_cres)}")

        if args.parallel > 1:
            plot_args = []
            for info in sig_cres:
                output_file = f"{output_dir}/individual_Emx1_{info['gene']}_{info['cre_id']}.png"
                args_tuple = (bw_emx1_ctrl, bw_emx1_mut, info['chrom'], info['start'], info['end'],
                             window, bins, info['gene'], info['cre_id'], 'Emx1', output_file,
                             args.individual_dpi)
                plot_args.append(args_tuple)
            ...
            for j, info in enumerate(sig_cres):
                output_file = f"{output_dir}/individual_Emx1_{info['gene']}_{info['cre_id']}.png"
                _plot_single_cre_bigwig((bw_emx1_ctrl, bw_emx1_mut, info['chrom'], info['start'], info['end'],
                                        window, bins, info['gene'], info['cre_id'], 'Emx1', output_file,
                                        args.individual_dpi))
                if (j + 1) % 20 == 0:
                    print(f"    Progress: {j+1}/{len(sig_cres)} plots")

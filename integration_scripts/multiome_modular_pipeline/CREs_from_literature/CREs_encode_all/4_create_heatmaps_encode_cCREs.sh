...
for SAMPLE in "GABA_Nestin-Ctrl" "GABA_Nestin-Mut" "GABA_Emx1-Mut"; do
    ...
    if [ -f "$BW_FILE" ]; then
        ...
        # Extract label from filename (remove GABA_ prefix)
        LABEL=${SAMPLE#GABA_}
        LABELS="$LABELS $LABEL"
...
for BED_FILE in $BED_DIR/encode_cCREs_*.bed; do
    ...
    # Skip all.bed (already processed)
    if [[ "$BASENAME" == "encode_cCREs_all" ]]; then
        continue
    fi
...
BED_GABA="$BED_DIR/encode_cCREs_all.bed"
...
if [ ! -f "$BED_GABA" ]; then
    ...
    # Emx1-Mut analysis (using Nestin-Ctrl as reference)
    if [ -f "$BIGWIG_BASE/GABA_Nestin-Ctrl.bw" ] && [ -f "$BIGWIG_BASE/GABA_Emx1-Mut.bw" ]; then
        echo "Creating Emx1-Mut analysis (vs Nestin-Ctrl)..."
        ...
        plotHeatmap \
            ...
            --heatmapHeight 10 \
            --heatmapWidth 3 \
            --plotTitle "Nestin-Ctrl vs Emx1-Mut: ATAC Signal (n=$N_GABA_CRES)" \
            --xAxisLabel "Distance from CRE Center (bp)" \
            2>&1

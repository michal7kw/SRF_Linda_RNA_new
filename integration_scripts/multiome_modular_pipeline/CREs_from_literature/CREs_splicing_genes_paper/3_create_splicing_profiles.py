#!/usr/bin/env python3
"""
Create Publication-Quality ATAC Signal Profiles for Splicing Gene CREs

This script creates enhanced visualizations of ATAC-seq signal at CREs linked
...
def read_deeptools_matrix(matrix_file):
    ...
    for line in data_lines:
        if line.strip():
            ...
            # First 6 columns might be BED info, but in this format they're just position indices
            # The actual data starts right away
...
def plot_individual_cres(data, regions, bin_labels, bed_file, output_dir,
    ...
    -----------
    n_processes : int
        Number of parallel processes to use (default: 1 for sequential)
    dpi : int
        DPI for individual plots (default: 150 to save time)
    """
    ...
    tsv_file = bed_file.replace('.bed', '.tsv').replace('splicing_genes_CREs_GABA',
                                                        'splicing_genes_CREs_all_celltypes')
    gene_map = {}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Create ATAC signal profiles for splicing gene CREs',

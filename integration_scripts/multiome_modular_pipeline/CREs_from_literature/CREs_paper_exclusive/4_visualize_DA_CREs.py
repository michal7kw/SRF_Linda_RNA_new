...
This script creates individual CRE plots for cell-type-specific CREs that show
...
- min_fc: Minimum fold change required (default: 1.5)
  - FC >= min_fc (increased accessibility)
  - FC <= 1/min_fc (decreased accessibility)
...
Usage:
    python 4_visualize_DA_CREs.py --skip-individual        # Fast mode
    python 4_visualize_DA_CREs.py --parallel 8             # Full with parallelization
    python 4_visualize_DA_CREs.py --min-signal 2.0 --min-fc 2.0  # Stricter filtering
...
def filter_significant_cres(data, regions, gene_info, sample1_name, sample2_name,
    ...
    Returns indices of significant CREs.
    """
    significant_indices = []
    stats_list = []
...
def _plot_single_cre(args):
    ...
    # Truncate gene list if too long
...
def main():
    ...
    parser.add_argument('--min-signal', type=float, default=1.0, metavar='F',
                        help='Minimum max signal required (default: 1.0)')
    ...
    if args.skip_individual:
        ...
        print(f"  Individual DPI: {args.individual_dpi}")
        print(f"  Min Signal: {args.min_signal}")
        print(f"  Min FC: {args.min_fc}")
    print()
    ...
    # CRE types to process
    cre_types = [
        {
            'name': 'GABA_specific',
            'matrix': f"{matrix_dir}/matrix_GABA_specific.gz",
            'bed': "output/GABA_specific_CREs.bed",
            'genes_tsv': "output/GABA_specific_CREs_genes.tsv",
            'label': 'GABA-specific'
        },
        {
            'name': 'Excitatory_specific',
            'matrix': f"{matrix_dir}/matrix_Excitatory_specific.gz",
            'bed': "output/Excitatory_specific_CREs.bed",
            'genes_tsv': "output/Excitatory_specific_CREs_genes.tsv",
            'label': 'Excitatory-specific'
        }
    ]
    ...
    for cre_type in cre_types:
        ...
        if not os.path.exists(cre_type['matrix'].replace('.gz', '.tab')):
            ...
            print(f"  Please run 1b_create_heatmaps_specific_CREs_deeptools.sh first")
            continue
        ...
        if os.path.exists(cre_type['genes_tsv']):
            ...
            print(f"  Please run 3_link_CREs_to_genes.py first")
        ...
        for comp in comparisons:
            ...
            print(f"CRE Type: {cre_type['label']}")
            print("="*80)
            ...
            if not args.skip_individual:
                ...
                print(f"\n  Skipping individual plots (fast mode)")
    ...
    with open(stats_file, 'w') as f:
        ...
        for cre_type_name, comp_stats in all_stats.items():
            f.write(f"\n{'#'*80}\n")
            f.write(f"# {cre_type_name.upper()}\n")
            f.write(f"{'#'*80}\n\n")

            for comp_name, stats_list in comp_stats.items():
                f.write(f"Comparison: {comp_name}\n")
                f.write(f"  Significant CREs: {len(stats_list)}\n")
                if stats_list:
                    avg_fc = np.mean([s['fold_change'] for s in stats_list])
                    f.write(f"  Average FC: {avg_fc:.3f}\n")
                f.write("\n")

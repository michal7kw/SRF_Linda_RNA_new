...
vmax_heatmap = vmax
print(f"Heatmap scale: 0 to {vmax_heatmap:.4f}")

# Calculate metaprofile scale (based on MEAN signal, not individual pixels)
max_mean_signal = 0
for matrix in list(signals_gaba.values()) + list(signals_excitatory.values()):
    mean_sig = np.mean(matrix, axis=0)
    max_mean_signal = max(max_mean_signal, np.max(mean_sig))

if max_mean_signal > 0:
    ymax_metaprofile = max_mean_signal * 1.2  # 20% buffer
else:
    ymax_metaprofile = 1.0

print(f"Metaprofile scale: 0 to {ymax_metaprofile:.4f}")
...
ax.set_ylim(0, ymax_metaprofile)
...
with open(stats_file, 'w') as f:
    ...
    f.write(f"SCALE:\n")
    f.write(f"  Heatmap scale: 0 to {vmax_heatmap:.4f} (90th percentile + 20%)\n")
    f.write(f"  Metaprofile scale: 0 to {ymax_metaprofile:.4f} (Max mean + 20%)\n\n")
...
print(f"\nHeatmaps use scale: 0-{vmax_heatmap:.4f}")
print(f"Metaprofiles use scale: 0-{ymax_metaprofile:.4f}")
print(f"\nExpected result:")

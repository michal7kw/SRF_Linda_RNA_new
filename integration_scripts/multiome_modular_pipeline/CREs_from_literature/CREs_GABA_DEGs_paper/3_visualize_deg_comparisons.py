#!/usr/bin/env python3
"""
Create Custom Comparison Visualizations for GABA DEG CREs

This script creates visualizations for ATAC signal at CREs linked to
GABA differentially expressed genes (up-regulated and down-regulated separately).

For each DEG set (up/down), creates three comparisons:
1. Nestin-Ctrl vs Nestin-Mut
2. Nestin-Ctrl vs Emx1-Mut
3. Nestin-Mut vs Emx1-Mut

INPUT:
- deepTools matrix files (matrix_up_*.tab, matrix_down_*.tab)
- BED files with CRE coordinates

OUTPUT:
- Publication-quality metaprofiles for each comparison (6 total)
- Difference plots showing condition effects
- Statistical summaries

PERFORMANCE OPTIONS:
- --skip-individual: Skip individual CRE plots (saves significant time)
...
def plot_individual_cres_comparison(data, regions, bin_labels, bed_file, tsv_file, output_dir,
                                    sample1_name, sample2_name, color1, color2,
...
def calculate_comparison_statistics(data, sample1_name, sample2_name):
    ...
    stats_dict['n_cres'] = len(sample1_cre_means)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Create custom comparison visualizations for GABA DEG CREs',
...
Examples:
  ...
  python %(prog)s --parallel 8 --individual-dpi 300
    ...
    parser.add_argument('--skip-individual', action='store_true',
                       help='Skip individual CRE plots (saves significant time)')
    ...
    print("="*80)
    print("CUSTOM COMPARISON VISUALIZATIONS FOR GABA DEG CREs")
    print("="*80)
    print()
    ...
    output_dir_up = f"{matrix_dir}/profiles_up{suffix}"
    output_dir_down = f"{matrix_dir}/profiles_down{suffix}"
    ...
    comparisons = [
        {
            ...
            'matrix_suffix': 'nestin_ctrl_vs_mut',
            'sample1': 'Nestin-Ctrl',
            'sample2': 'Nestin-Mut',
            'color1': '#2E86AB',  # Blue
            'color2': '#C73E1D',  # Red
        ...
        },
        {
            ...
            'matrix_suffix': 'nestin_ctrl_vs_emx1_mut',
            'sample1': 'Nestin-Ctrl',
            'sample2': 'Emx1-Mut',
            'color1': '#2E86AB',  # Blue
        ...
        },
        {
            ...
            'matrix_suffix': 'nestin_mut_vs_emx1_mut',
            'sample1': 'Nestin-Mut',
            'sample2': 'Emx1-Mut',
            'color1': '#C73E1D',  # Red
            'color2': '#FF8C42',  # Orange
    ...
    # DEG types to process
    deg_types = [
        {'prefix': 'up', 'output_dir': output_dir_up, 'bed': './output/GABA_DEGs_up_CREs.bed',
         'tsv': './output/GABA_DEGs_up_CREs.tsv', 'label': 'Up-regulated', 'title_suffix': 'Up-regulated DEGs'},
        {'prefix': 'down', 'output_dir': output_dir_down, 'bed': './output/GABA_DEGs_down_CREs.bed',
         'tsv': './output/GABA_DEGs_down_CREs.tsv', 'label': 'Down-regulated', 'title_suffix': 'Down-regulated DEGs'}
    ]

    # Process each DEG type
    all_stats = {}

    for deg_type in deg_types:
        print("\n" + "#"*80)
        print(f"# PROCESSING {deg_type['label'].upper()} DEGs")
        print("#"*80)

        # Check if BED file exists
        if not os.path.exists(deg_type['bed']):
            print(f"  WARNING: BED file not found: {deg_type['bed']}")
            print(f"  Skipping {deg_type['label']} DEGs")
            continue

        all_stats[deg_type['prefix']] = {}

        for comp in comparisons:
            print("\n" + "="*80)
            print(f"COMPARISON: {comp['description'].upper()}")
            print(f"({comp['sample1']} vs {comp['sample2']})")
            print(f"DEG Type: {deg_type['label']}")
            print("="*80)

            matrix_file = f"{matrix_dir}/matrix_{deg_type['prefix']}_{comp['matrix_suffix']}.gz"
            ...
            plot_comparison_metaprofile(
                data, bin_labels,
                f"{deg_type['output_dir']}/metaprofile_{comp['name']}.png",
                f"{comp['title']}\nATAC Signal at {deg_type['title_suffix']} CREs",
                comp['sample1'], comp['sample2'],
                comp['color1'], comp['color2'],
                deg_type=deg_type['label']
            )
            ...
            if not args.skip_individual:
                ...
                plot_individual_cres_comparison(
                    data, regions, bin_labels, deg_type['bed'], deg_type['tsv'],
                    deg_type['output_dir'],
                    comp['sample1'], comp['sample2'],
                    comp['color1'], comp['color2'],
                    f"{deg_type['prefix']}_{comp['name']}",
            ...
            all_stats[deg_type['prefix']][comp['name']] = stats_result

            print(f"\n{comp['description']} Statistics ({deg_type['label']}):")
            print(f"  Number of CREs: {stats_result['n_cres']}")
    ...
    print("="*80)
    print(f"\nOutput directories:")
    print(f"  Up-regulated DEGs: {output_dir_up}/")
    print(f"  Down-regulated DEGs: {output_dir_down}/")
    print("\nGenerated files:")
    print("  Metaprofiles (always created):")
    for deg_type in deg_types:
        print(f"    {deg_type['label']}:")
        for comp in comparisons:
            print(f"      - metaprofile_{comp['name']}.png")

    if not args.skip_individual:
        print(f"\n  Individual CRE plots (DPI: {args.individual_dpi}):")
        for deg_type in deg_types:
            if os.path.exists(deg_type['bed']):
                bed_df = pd.read_csv(deg_type['bed'], sep='\t', header=None)
                n_cres = len(bed_df)
                print(f"    {deg_type['label']}: {n_cres * 3} plots ({n_cres} CREs x 3 comparisons)")
        if args.parallel > 1:
    ...
    with open(stats_file, 'w') as f:
        f.write("="*80 + "\n")
        f.write("GABA DEG CRE COMPARISON STATISTICS\n")
        f.write("="*80 + "\n\n")

        for deg_type in deg_types:
            f.write(f"\n{'#'*80}\n")
            f.write(f"# {deg_type['label'].upper()} DEGs\n")
            f.write(f"{'#'*80}\n\n")

            if deg_type['prefix'] not in all_stats:
                f.write(f"  No data available\n")
                continue

            for comp in comparisons:
                if comp['name'] in all_stats[deg_type['prefix']]:
                    s = all_stats[deg_type['prefix']][comp['name']]
                    f.write(f"{comp['description'].upper()}\n")
                    f.write(f"({comp['sample1']} vs {comp['sample2']})\n")
                    f.write("-"*80 + "\n")
                    f.write(f"  Number of CREs: {s['n_cres']}\n")

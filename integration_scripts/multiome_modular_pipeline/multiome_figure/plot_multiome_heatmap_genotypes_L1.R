...
################################################################################
# Plot Multiome Heatmaps - Genotype-Stratified Version (L1 VERSION)
################################################################################
#
# Creates separate heatmaps for Nestin and Emx1 genotypes showing:
# - RNA expression changes (DEGs) for L1 cell types
# - ATAC gene activity changes (chromatin accessibility)
# - Side-by-side comparison for concordant chromatin-driven genes
#
# L1 VERSION: Visualizes 9 broad cell categories
# Input: multiome_heatmap_data_L1_{Genotype}.rds
# Output: output_genotypes_L1/
#
#
# Input Files:
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/multiome_figure/multiome_heatmap_data_L1_{Genotype}.rds
#     Processed data for each genotype containing RNA and ATAC matrices with z-scores
#     (Genotype = Nestin or Emx1)
#
# Output Files:
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/multiome_figure/output_genotypes_L1/multiome_heatmap_L1_{Genotype}.pdf
#     Combined RNA+ATAC heatmaps for each genotype (L1 cell types)
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/multiome_figure/output_genotypes_L1/multiome_heatmap_L1_{Genotype}.png
...
cat("================================================================================\n")
cat("  PLOT MULTIOME HEATMAPS: GENOTYPE-STRATIFIED VISUALIZATION (L1 VERSION)\n")
cat("================================================================================\n\n")
...
OUTPUT_DIR <- file.path(FIGURE_DIR, "output_genotypes_L1")
...
create_genotype_heatmap <- function(geno) {
  ...
  # Load data
  data_file <- file.path(FIGURE_DIR, sprintf("multiome_heatmap_data_L1_%s.rds", geno))
  ...
  # Save to PDF
  output_file <- file.path(OUTPUT_DIR, sprintf("multiome_heatmap_L1_%s.pdf", geno))
  ...
  output_png <- file.path(OUTPUT_DIR, sprintf("multiome_heatmap_L1_%s.png", geno))
...
# Detect available genotypes (L1 version)
genotype_files <- list.files(FIGURE_DIR, pattern = "^multiome_heatmap_data_L1_.*\\.rds$", full.names = FALSE)
genotypes <- gsub("multiome_heatmap_data_L1_(.*)\\.rds", "\\1", genotype_files)

if (length(genotypes) == 0) {
  stop("No data files found. Please run prepare_multiome_heatmap_data_genotypes_L1.R first.")
}

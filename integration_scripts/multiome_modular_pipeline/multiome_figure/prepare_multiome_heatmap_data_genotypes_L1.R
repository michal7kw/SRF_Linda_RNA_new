...
################################################################################
# Prepare Multiome Heatmap Data - Genotype-Stratified Version (L1 VERSION)
################################################################################
#
...
#   3. Select TOP L1 cell types by abundance (automatic detection)
...
#
# Output: Separate .rds files for Nestin and Emx1 in output_genotypes_L1/
#
#
# Input Files:
...
#     Integrated Seurat object with processed RNA and ATAC data (L1 annotations)
...
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/multiome_figure/output_genotypes_L1/chromatin_driven_degs_L1_{Genotype}.csv
...
cat("================================================================================\n")
cat("  PREPARE MULTIOME HEATMAP DATA: GENOTYPE-STRATIFIED ANALYSIS (L1 VERSION)\n")
cat("================================================================================\n\n")
...
RESULTS_DIR <- file.path(BASE_DIR, "signac_results_L1/celltype_results")
...
seurat_file <- file.path(BASE_DIR, "signac_results_L1/integrated_seurat_processed.rds")

if (!file.exists(seurat_file)) {
  stop(sprintf("Seurat object not found: %s\nPlease run signac_02_process_and_reduce_L1.R first.", seurat_file))
}
...
for (geno in genotypes) {
  ...
  if (!is.null(geno_results)) {
    ...
    output_file <- file.path(FIGURE_DIR, sprintf("multiome_heatmap_data_L1_%s.rds", geno))
    saveRDS(geno_results, output_file)
    cat(sprintf("\n✓ Saved %s results to: %s\n", geno, output_file))

    # Save summary CSV
    summary_file <- file.path(OUTPUT_DIR, sprintf("chromatin_driven_degs_L1_%s.csv", geno))

#!/usr/bin/env python3
"""
Script to generate marker gene expression stacked violin plots for cell_type_L1 annotations
...
OUTPUT_DIR = '/beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/ADDITIONAL_PLOTS/stacked_violins/L1'
CELL_TYPE_COLUMN = 'cell_type_L1'
# Generate multiple plots with different numbers of top markers
TOP_N_GENES_LIST = [3, 6]  # Will create separate plots for each
...
def create_stacked_violin(adata, marker_genes_dict, output_dir, top_n_genes, cell_type_column='cell_type_L1'):
    ...
    # Figure sizing - L1 has only 9 cell types
    ...
    for swap_axes in [False, True]:
        try:
            if swap_axes:
                ...
                fig_width = max(12, n_cell_types * 1.2)
                fig_height = max(18, n_genes * 0.6)
                fontsize = 10
                suffix = "_swapped"
            else:
            ...
            if fig is not None and hasattr(fig, 'get_axes') and len(fig.get_axes()) > 0:
                ...
                fig.suptitle(f'Cell Type Markers (L1) - Stacked Violin {orientation_text}\nTop {top_n_genes} per cell type',
...
def main():
    ...
    print("="*80)
    print("CELL TYPE L1 MARKER GENE STACKED VIOLIN PLOT GENERATION")
    print("="*80)

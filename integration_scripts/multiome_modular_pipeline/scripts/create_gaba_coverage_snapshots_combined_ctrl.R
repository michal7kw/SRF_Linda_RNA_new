...
################################################################################
# GABA Coverage Snapshots - Combined Controls Version
################################################################################
#
...
#     /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/plots/gaba_coverage_snapshots_combined_ctrl/GABA_coverage_snapshots_combined_controls.pdf
#   - Individual gene PDFs:
#     /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/plots/gaba_coverage_snapshots_combined_ctrl/GABA_coverage_<gene>_combined_ctrl.pdf
#   - Summary PDF with all genes by comparison:
#     /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/plots/gaba_coverage_snapshots_combined_ctrl/GABA_coverage_summary_all_genes_combined_ctrl.pdf
#
################################################################################
...
cat("================================================================================\n")
cat("  GABA COVERAGE SNAPSHOTS - COMBINED CONTROLS VERSION\n")
cat("================================================================================\n\n")
...
OUTPUT_DIR <- file.path(INPUT_DIR, "plots", "gaba_coverage_snapshots_combined_ctrl")
...
seurat_gaba@meta.data$combined_group <- ifelse(
  ...
  paste(seurat_gaba@meta.data[[GENOTYPE_COL]], "Mut", sep = "_")  # Keep mutants separate
)

# Show breakdown by combined group
combined_breakdown <- seurat_gaba@meta.data %>%
...
for (i in 1:nrow(combined_breakdown)) {
  cat(sprintf("    %s: %d cells (Nestin: %d, Emx1: %d)\n",
...
create_gene_coverage_plot_combined <- function(seurat_obj, gene, mutant_genotype,
  ...
  p <- tryCatch({
    ...
    coverage_plot +
      plot_annotation(
        title = sprintf("%s - Combined Controls vs %s Mut (GABA neurons)", gene, mutant_genotype),
...
for (gene in GENES_OF_INTEREST) {
  ...
  if (!is.null(plot_nestin)) {
    output_file_nestin <- file.path(OUTPUT_DIR,
                                     sprintf("GABA_coverage_%s_vs_Nestin_Mut.png", gene))
  ...
  if (!is.null(plot_emx1)) {
    output_file_emx1 <- file.path(OUTPUT_DIR,
                                   sprintf("GABA_coverage_%s_vs_Emx1_Mut.png", gene))
...
#   output_nestin_summary <- file.path(OUTPUT_DIR, "GABA_coverage_all_genes_vs_Nestin_Mut.png")
...
#     plot_annotation(
#       title = "Combined Controls vs Nestin Mut - All Genes (GABA neurons)",
...
#   output_emx1_summary <- file.path(OUTPUT_DIR, "GABA_coverage_all_genes_vs_Emx1_Mut.png")
...
#     plot_annotation(
#       title = "Combined Controls vs Emx1 Mut - All Genes (GABA neurons)",
#       theme = theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 16))
#     )
#   print(combined_plot)
...
cat(sprintf("  %-18s %8s %10s %10s\n",
            "Group", "Total", "Nestin", "Emx1"))
cat(strrep("-", 50), "\n")

for (i in 1:nrow(summary_table)) {
  cat(sprintf("  %-18s %8d %10d %10d\n",
...
cat(sprintf("  - %s/GABA_coverage_<gene>_vs_Nestin_Mut.png (individual comparisons)\n", OUTPUT_DIR))
cat(sprintf("  - %s/GABA_coverage_<gene>_vs_Emx1_Mut.png (individual comparisons)\n", OUTPUT_DIR))
cat(sprintf("  - %s/GABA_coverage_all_genes_vs_Nestin_Mut.png (all genes summary)\n", OUTPUT_DIR))
cat(sprintf("  - %s/GABA_coverage_all_genes_vs_Emx1_Mut.png (all genes summary)\n", OUTPUT_DIR))

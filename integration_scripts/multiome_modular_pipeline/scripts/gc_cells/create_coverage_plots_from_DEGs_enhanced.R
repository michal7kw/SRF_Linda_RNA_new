...
################################################################################
# Enhanced Coverage Plots for Significant DEGs (Combined Controls - L2 Version)
# REFACTORED VERSION - Uses Pre-computed Links
#
# This script creates comprehensive visualizations for genes identified as
# significant in differential expression analysis using L2 (fine-grained) cell types.
#
...
# 3. Pre-computed peak-gene linkage from array jobs
...
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/02_processed_seurat.rds
#
# Output:
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/DEG_coverage_plots_enhanced/fragment_abundance/{celltype}_fragment_abundance_combined.pdf
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/DEG_coverage_plots_enhanced/{celltype}_{genotype}/{gene}_coverage_combined.pdf
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/DEG_coverage_plots_enhanced/peak_gene_links/{celltype}_{genotype}_{gene}_links.csv
################################################################################
...
# Cell types to analyze (L2 annotation - high-priority types with both genotypes)
CELL_TYPES <- c("Astrocytes", "CA1", "CTX", "Mature GC", "ENT CTX")
...
cat("================================================================================\n")
cat("ENHANCED DEG COVERAGE PLOTS - L2 CELL TYPES (Combined Controls)\n")
cat("================================================================================\n\n")
...
if (!dir.exists(LINKS_DIR)) {
  ...
  cat("  Option 1 (High Priority): sbatch run_peak_gene_linkage_high_priority.sh\n")
  cat("  Option 2 (All Cell Types): sbatch run_peak_gene_linkage_all.sh\n\n")
...
if ("nCount_ATAC" %in% colnames(seurat_obj@meta.data)) {
  for (celltype in CELL_TYPES) {
    ...
    p1 <- ggplot(plot_data, aes(x = combined_group, y = log10_fragments, fill = combined_group)) +
      ...
      labs(
        title = sprintf("ATAC Fragment Counts - %s (L2)", celltype),
  ...
  for (genotype in GENOTYPES) {
    ...
    for (i in 1:n_plot) {
      ...
      if (nrow(gene_links) > 0) {
        cat(sprintf("      Found %d pre-computed links\n", nrow(gene_links)))
      ...
      tryCatch({
        ...
        # Add title
        plot_title <- sprintf("%s (L2) - %s %s\nlog2FC = %.2f, FDR = %.2e (%s)\n%d pre-computed links",
...
cat(sprintf("\nNOTE: All peak-gene links were pre-computed by array jobs.\n"))

...
################################################################################
# Peak-Gene Linkage Analysis for Granule Cells - Separate Genotypes
# (UPDATED VERSION - Using Pre-computed Links)
#
# This is the separate genotypes version (4 groups).
# KEY CHANGE: Loads pre-computed LinkPeaks from array job outputs instead of manual cor.test()
#
# COMPATIBILITY NOTE: This script expects L2 cell types (Immature GC, Mature GC).
# Array jobs currently generate L1 cell types only. For L1 analysis, use
# create_coverage_plots_from_DEGs_enhanced_separate.R with Excitatory neurons.
#
...
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/celltype_results/peak_gene_links/*GC*_peak_gene_links.csv (L2 preferred)
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/celltype_results/peak_gene_links/*GC*_peak_gene_links.csv (L1 fallback)
#
# Output:
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/peak_gene_linkage_analysis_UPDATED/GC_separate/peak_gene_links/{gene}_links_separate.csv
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/peak_gene_linkage_analysis_UPDATED/GC_separate/coverage_plots/{gene}_coverage_separate.pdf
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/peak_gene_linkage_analysis_UPDATED/GC_separate/analysis_summary_separate.csv
################################################################################
...
OUTPUT_DIR <- file.path(RESULTS_DIR, "peak_gene_linkage_analysis_UPDATED", "GC_separate")

# Try L2 results first (preferred), fallback to L1
STEP3_LINKS_DIR_L2 <- file.path("signac_results", "celltype_results", "peak_gene_links")
STEP3_LINKS_DIR_L1 <- file.path(RESULTS_DIR, "celltype_results", "peak_gene_links")

# Cell type columns
CELLTYPE_COL <- "cell_type_L2_new"
GC_CELL_TYPES <- c("Immature GC", "Mature GC", "GC")
...
cat("================================================================================\n")
cat("GRANULE CELLS: Separate Genotypes (Using Pre-computed Links from Step 3)\n")
cat("================================================================================\n\n")
...
if (length(gc_files) == 0) {
  cat("\n✗ ERROR: No GC LinkPeaks files found!\n\n")
  cat("This script expects L2 cell type files (Immature GC, Mature GC).\n")
  cat("Current array jobs generate L1 cell types only.\n\n")
  cat("ALTERNATIVES:\n")
  cat("  1. For L1 analysis, use: create_coverage_plots_from_DEGs_enhanced_separate.R\n")
...
for (file in gc_files) {
  ...
  if (!"genotype" %in% colnames(df)) {
    # If missing, extract from filename
...
seurat_gc@meta.data$genotype_condition <- paste0(
...
Idents(seurat_gc) <- seurat_gc@meta.data$genotype_condition

...
################################################################################
# GC Coverage Snapshots for Specific Genes
################################################################################
#
# This script creates publication-quality chromatin accessibility coverage
# plots for Immature GC + Mature GC combined, showing:
#   - Genes: Read from genes.txt file (10 genes)
...
#     /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/plots/gc_coverage_snapshots/GC_coverage_snapshots_specific_genes.pdf
#   - Individual gene PDFs:
#     /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/plots/gc_coverage_snapshots/GC_coverage_<gene>.pdf
#   - Summary PDF with all genes by genotype:
#     /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/plots/gc_coverage_snapshots/GC_coverage_summary_all_genes.pdf
#
################################################################################
...
cat("================================================================================\n")
cat("  GC COVERAGE SNAPSHOTS - SPECIFIC GENES\n")
cat("================================================================================\n\n")
...
OUTPUT_DIR <- file.path(INPUT_DIR, "plots", "gc_coverage_snapshots")
...
create_gene_coverage_plot <- function(seurat_obj, gene, genotype,
  ...
  p <- tryCatch({
    ...
    coverage_plot +
      plot_annotation(
        title = sprintf("%s - %s Genotype (GC: Immature + Mature)", gene, genotype),
...
for (gene in GENES_OF_INTEREST) {
  ...
  if (!is.null(plot_nestin)) {
    output_file_nestin <- file.path(OUTPUT_DIR,
                                     sprintf("GC_coverage_%s_Nestin.png", gene))
  ...
  if (!is.null(plot_emx1)) {
    output_file_emx1 <- file.path(OUTPUT_DIR,
                                   sprintf("GC_coverage_%s_Emx1.png", gene))
...
if (length(nestin_plots) > 0) {
  output_nestin_summary <- file.path(OUTPUT_DIR, "GC_coverage_all_genes_Nestin.png")
  ...
  combined_plot <- wrap_plots(nestin_plots, ncol = 1) +
    plot_annotation(
      title = "Nestin Genotype (Ctrl vs Mut) - All Genes (GC: Immature + Mature)",
...
if (length(emx1_plots) > 0) {
  output_emx1_summary <- file.path(OUTPUT_DIR, "GC_coverage_all_genes_Emx1.png")
  ...
  combined_plot <- wrap_plots(emx1_plots, ncol = 1) +
    plot_annotation(
      title = "Emx1 Genotype (Ctrl vs Mut) - All Genes (GC: Immature + Mature)",
      theme = theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 16))
    )
  print(combined_plot)
...
cat("\nBreakdown:\n")
cat(sprintf("  %-10s %-10s %8s %10s %10s\n",
            "Genotype", "Condition", "Total", "Immature", "Mature"))
cat(strrep("-", 55), "\n")

for (i in 1:nrow(summary_table)) {
  cat(sprintf("  %-10s %-10s %8d %10d %10d\n",
...
cat(sprintf("  - %s/GC_coverage_<gene>_Nestin.png (individual comparisons)\n", OUTPUT_DIR))
cat(sprintf("  - %s/GC_coverage_<gene>_Emx1.png (individual comparisons)\n", OUTPUT_DIR))
cat(sprintf("  - %s/GC_coverage_all_genes_Nestin.png (all genes summary)\n", OUTPUT_DIR))
cat(sprintf("  - %s/GC_coverage_all_genes_Emx1.png (all genes summary)\n", OUTPUT_DIR))

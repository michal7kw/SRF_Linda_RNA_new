...
################################################################################
# Coverage and Tile Plot Visualization for GABA Neurons (Separate Genotypes)
#
...
#     /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/visualizations_separate/GABA/coverage_no_peaks/<gene>_coverage_clean_separate.pdf
#   - Tile plots for each genotype group:
#     /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/visualizations_separate/GABA/tile_plots/<gene>_tile_<group>.pdf
#   - Summary CSV with processing status:
#     /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/visualizations_separate/GABA/visualization_summary_separate.csv
#
################################################################################
...
OUTPUT_DIR <- file.path(RESULTS_DIR, "visualizations_separate", "GABA")

CELLTYPE_COL <- "cell_type_L1"; CELL_TYPE <- "GABA"
...
cat("================================================================================\n")
cat("COVERAGE AND TILE VISUALIZATION: GABA NEURONS (SEPARATE GENOTYPES)\n")
cat("================================================================================\n\n")
...
seurat_gaba@meta.data$genotype_condition <- paste(
  ...
  seurat_gaba@meta.data[[CONDITION_COL]], sep = "-"
)
...
for (gene in genes_of_interest) {
  ...
  tryCatch({
    ...
    p_coverage <- CoveragePlot(
      object = seurat_gaba, region = gene, features = gene,
    ...
    ggsave(file.path(OUTPUT_DIR, "coverage_no_peaks", sprintf("%s_coverage_clean_separate.pdf", gene)),
           plot = p_coverage + ggtitle(sprintf("GABA - %s (Separate Genotypes)", gene)),
  ...
  Idents(seurat_gaba) <- "genotype_condition"

  for (group in GROUPS) {
    tryCatch({
      DefaultAssay(seurat_gaba) <- "ATAC"
      p_tile <- TilePlot(object = seurat_gaba, region = gene, idents = group,
                        extend.upstream = EXTEND_UPSTREAM,
                        extend.downstream = EXTEND_DOWNSTREAM)
      ggsave(file.path(OUTPUT_DIR, "tile_plots", sprintf("%s_tile_%s.pdf", gene, group)),
             plot = p_tile + ggtitle(sprintf("GABA - %s - %s", gene, group)),

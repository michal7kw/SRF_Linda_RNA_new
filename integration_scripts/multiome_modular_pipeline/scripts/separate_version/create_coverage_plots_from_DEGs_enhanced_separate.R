...
################################################################################
# Enhanced Coverage Plots for Significant DEGs from Step 3 (Separate Genotypes)
# REFACTORED VERSION - Uses Pre-computed Links
#
# This script creates comprehensive visualizations for genes identified as
# significant in Step 3 differential expression analysis, with separate genotypes.
#
...
# 3. Pre-computed peak-gene linkage from array job outputs
# 4. Genomic links visualization (arcs connecting peaks to genes)
#
# Input:
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/celltype_results/DEG/{celltype}_{genotype}_DEGs.csv (from Step 3)
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/celltype_results/peak_gene_links/{celltype}_{genotype}_peak_gene_links.csv (from array jobs)
...
#
# Output:
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/DEG_coverage_plots_enhanced_separate/fragment_abundance/{celltype}_fragment_abundance_separate.pdf
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/DEG_coverage_plots_enhanced_separate/{celltype}_{genotype}/{gene}_coverage_separate.pdf
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results/DEG_coverage_plots_enhanced_separate/peak_gene_links/{celltype}_{genotype}_{gene}_links.csv
################################################################################
...
SEURAT_FILE <- file.path(RESULTS_DIR, "02_processed_seurat.rds")
DEG_DIR <- file.path(RESULTS_DIR, "celltype_results", "DEG")
LINKS_DIR <- file.path(RESULTS_DIR, "celltype_results", "peak_gene_links")
OUTPUT_DIR <- file.path(RESULTS_DIR, "DEG_coverage_plots_enhanced_separate")
...
cat("================================================================================\n")
cat("ENHANCED DEG COVERAGE PLOTS - L2 CELL TYPES (Separate Genotypes - Using Pre-computed Links)\n")
cat("================================================================================\n\n")
cat("KEY CHANGE: Loads pre-computed LinkPeaks from Step 3 (NO manual cor.test())\n\n")
...
# Add separate genotype grouping (4 groups)
seurat_obj@meta.data$genotype_condition <- paste(
...
################################################################################
# Load Pre-computed Links from Step 3
################################################################################
...
if (!dir.exists(LINKS_DIR)) {
  ...
  cat("\nYou must run peak-gene linkage array jobs first:\n")
  ...
  cat("  Option 3 (Individual): sbatch run_signac_peak_gene_linkage_single_L1.sh <CELLTYPE> <GENOTYPE>\n\n")
  stop("Pre-computed links not found. Run peak-gene linkage array jobs first.")
}
...
if (length(all_links) == 0) {
  cat("\n✗ ERROR: No pre-computed links found for any cell type!\n")
  cat("\nYou must run peak-gene linkage array jobs first:\n")
  cat("  sbatch run_peak_gene_linkage_high_priority.sh   # For GABA, Excitatory, Oligo, Immune\n")
  cat("  sbatch run_peak_gene_linkage_all.sh             # For all L1 cell types\n\n")
  stop("No pre-computed links found. Run peak-gene linkage array jobs first.")
}

# Combine all links
all_links_df <- do.call(rbind, all_links)
cat(sprintf("\n✓ Total pre-computed links loaded: %s\n", format(nrow(all_links_df), big.mark = ",")))
cat(sprintf("  Unique genes with links: %d\n", length(unique(all_links_df$gene))))
cat(sprintf("  Cell type × genotype combinations with links: %d\n", length(all_links)))

# Summary by cell type and genotype
cat("\n  Links per cell type × genotype:\n")
link_summary <- all_links_df %>%
  group_by(celltype, genotype) %>%
  summarise(n_links = n(), n_genes = n_distinct(gene), .groups = "drop")
print(link_summary)
...
if ("nCount_ATAC" %in% colnames(seurat_obj@meta.data)) {
  for (celltype in CELL_TYPES) {
    ...
    pdf_file <- file.path(OUTPUT_DIR, "fragment_abundance",
                         sprintf("%s_fragment_abundance_separate.pdf", celltype))
  ...
  for (genotype in GENOTYPES) {
    ...
    for (i in 1:n_plot) {
      ...
      if (nrow(gene_links) > 0) {
        cat(sprintf("      Found %d pre-computed links for %s (from Step 3)\n",
                    nrow(gene_links), genotype))
      ...
      # Create coverage plot with all 4 groups
      tryCatch({

...
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/peak_gene_linkage_analysis/GABA/{gene}_peak_gene_links.csv
#
# Output:
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/peak_gene_linkage_analysis/GABA/visualizations/coverage_{gene}_gaba.pdf
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/peak_gene_linkage_analysis/GABA/visualizations/fragment_abundance_gaba.pdf
#   - /beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/integration_scripts/multiome_modular_pipeline/signac_results_L1/peak_gene_linkage_analysis/GABA/visualizations/correlation_scatter_{gene}_gaba.pdf
################################################################################
...
LINKAGE_DIR <- file.path(RESULTS_DIR, "peak_gene_linkage_analysis", "GABA")
...
cat("================================================================================\n")
cat("VISUALIZATION: PEAK-GENE LINKAGES - GABA NEURONS\n")
cat("================================================================================\n\n")
...
if ("nCount_ATAC" %in% colnames(seurat_gaba@meta.data)) {
  ...
  # Save combined plot
  pdf(file.path(OUTPUT_DIR, "fragment_abundance_gaba.pdf"), width = 14, height = 6)
...
for (gene in genes_of_interest) {
  ...
  tryCatch({
    ...
    # Save plot
    pdf_file <- file.path(OUTPUT_DIR, sprintf("coverage_%s_gaba.pdf", gene))
...
for (gene in genes_of_interest) {
  ...
  for (i in 1:min(6, nrow(top_links))) {
    ...
    peak_acc <- GetAssayData(seurat_gaba, assay = "ATAC", layer = "data")[peak, ]
    gene_expr <- GetAssayData(seurat_gaba, assay = "RNA", layer = "data")[gene, ]

    # Create data frame
    scatter_data <- data.frame(
      peak_accessibility = as.numeric(peak_acc),
      gene_expression = as.numeric(gene_expr),
      group = seurat_gaba@meta.data$combined_group
    )
  ...
  if (length(plots_list) > 0) {
    pdf_file <- file.path(OUTPUT_DIR, sprintf("correlation_scatter_%s_gaba.pdf", gene))

...
import matplotlib.patches as mpatches # Keep if plot_volcano needs it, remove otherwise if refactored
...
def save_significant_degs(result_df, base_output_dir, analysis_type, comparison_details, group_name,
                          comparison_name, padj_threshold=0.05, logfc_threshold=1.0):
    """
    ...
    # Ensure group_name is a plain string and log its initial state
    plain_group_name = str(group_name) # Ensure it's a string
    print(f"    DEBUG: Original group_name: '{plain_group_name}', type: {type(plain_group_name)}")

    # Sanitize group_name by whitelisting allowed characters
    allowed_chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-"
    sanitized_list = []
    for char in plain_group_name:
        if char in allowed_chars:
            sanitized_list.append(char)
        elif char == ' ': # Explicitly replace space with underscore
            sanitized_list.append('_')
        else: # Replace any other disallowed character with underscore
            sanitized_list.append('_')
    
    sanitized_group_name = "".join(sanitized_list)
    # Replace multiple underscores with a single one if they resulted from multiple replacements
    sanitized_group_name = re.sub(r'_+', '_', sanitized_group_name)
    # Remove leading/trailing underscores that might result
    sanitized_group_name = sanitized_group_name.strip('_')
    if not sanitized_group_name: # Handle case where name becomes empty
        sanitized_group_name = "unnamed_group"

    print(f"    DEBUG: Sanitized group_name (whitelisting): '{sanitized_group_name}'")
    ...
    up_genes = result_df.loc[up_mask, 'names'].tolist()
    ...
    if not up_df.empty:
        up_df.to_csv(str(up_filename), index=False)
        ...
        pd.DataFrame(columns=result_df.columns).to_csv(str(up_filename), index=False)
    ...
    if not down_df.empty:
        down_df.to_csv(str(down_filename), index=False)
        print(f"    Saved {len(down_df)} significant DOWN genes with stats to {down_filename}")
    else:
        # Create empty file with header
        pd.DataFrame(columns=result_df.columns).to_csv(str(down_filename), index=False)
...
def plot_volcano(result_df, group_name, n1, n2, output_dir, filename_prefix, group1_name="Mutant", group2_name="Control", title_prefix="",
                 padj_threshold=0.05, logfc_threshold=1.0, num_genes_to_label=5,
    ...
    output_filename = f"{filename_prefix}_{group_name}_volcano.png" # Use group_name in filename
    # Ensure output_dir exists before joining path
    # Use pathlib for robust path creation
    plot_output_path = pathlib.Path(output_dir) / output_filename
    plot_output_path.parent.mkdir(parents=True, exist_ok=True) # Ensure directory exists

    print(f"  Generating volcano plot for: {title_prefix}{group_name} (Counts: {group2_name}={n2}, {group1_name}={n1}) -> {output_filename}")

    # Prepare data for plotting
    df = result_df.copy()
    # print(f"    DEBUG: Columns in received result_df for volcano plot: {list(df.columns)}") # Removed DEBUG
    ...
    # Calculate average PTS for significant genes if 'pts' and 'pts_rest' columns exist
    pts_info_str = ""
    # Standard column names from Scanpy's pts=True
    pts_col_g1 = 'pts'
    pts_col_g2 = 'pts_rest'

    # Check for alternative column names if standard ones are not found
    if not (pts_col_g1 in df.columns and pts_col_g2 in df.columns):
        # print(f"    DEBUG: Standard PTS columns ('{pts_col_g1}', '{pts_col_g2}') not found. Checking for alternatives...") # Removed DEBUG
        if 'pct_nz_group' in df.columns and 'pct_nz_reference' in df.columns:
            pts_col_g1 = 'pct_nz_group'
            pts_col_g2 = 'pct_nz_reference'
            # print(f"    DEBUG: Using alternative PTS columns: '{pts_col_g1}', '{pts_col_g2}'") # Removed DEBUG
        else:
            # print(f"    DEBUG: Alternative PTS columns also not found. PTS info will not be added.") # Removed DEBUG
            # Set to None or handle so the next block doesn't error if they are used
            pts_col_g1 = None
            pts_col_g2 = None

    if pts_col_g1 and pts_col_g2 and pts_col_g1 in df.columns and pts_col_g2 in df.columns:
        avg_pts_up_g1 = df.loc[sig_up_mask, pts_col_g1].mean() if n_degs_up > 0 else 0
        avg_pts_up_g2 = df.loc[sig_up_mask, pts_col_g2].mean() if n_degs_up > 0 else 0
        avg_pts_down_g1 = df.loc[sig_down_mask, pts_col_g1].mean() if n_degs_down > 0 else 0
        avg_pts_down_g2 = df.loc[sig_down_mask, pts_col_g2].mean() if n_degs_down > 0 else 0

        pts_info_list = []
        if n_degs_up > 0:
            pts_info_list.append(f"Avg PTS Up ({group1_name} vs {group2_name}): {avg_pts_up_g1:.2f} vs {avg_pts_up_g2:.2f}")
        if n_degs_down > 0:
            # For "Down in group1", 'pts' still refers to group1, 'pts_rest' to group2
            # The interpretation is that these genes are lower in group1.
            # So, pts_down_g1 is the (lower) expression in group1, pts_down_g2 is the (higher) expression in group2.
            pts_info_list.append(f"Avg PTS Down ({group1_name} vs {group2_name}): {avg_pts_down_g1:.2f} vs {avg_pts_down_g2:.2f}")
        if pts_info_list:
            pts_info_str = "\n" + ", ".join(pts_info_list)
    else: # DEBUG
        # This else block is now covered by the logic above setting pts_col_g1/g2 to None
        pass # No specific message needed here if already handled by the DEBUG above
            
    # print(f"    DEBUG: Generated pts_info_str: '{pts_info_str}'") # Removed DEBUG
    # Create plot
    ...
    if pd.isna(max_finite_logp_label) or max_finite_logp_label == 0: max_finite_logp_label = 300
    ...
    # Save the figure
    try:
        fig.savefig(str(plot_output_path), bbox_inches='tight', dpi=300) # Convert Path to string for savefig
...
def run_overall_dge(adata, grouping_key, dge_output_dir, plot_output_dir, layer=None):
    ...
    # Use pathlib for robust path creation
    overall_dge_dir = pathlib.Path(dge_output_dir) / 'both_geno_cond_comp'
    overall_plot_dir = pathlib.Path(plot_output_dir) / 'both_geno_cond_comp'
    overall_dge_dir.mkdir(parents=True, exist_ok=True)
    ...
    for group_value in groups:
        ...
        # Skip if too few cells in either condition for comparison
        min_cells_per_group = 3
        if n_ctrl < min_cells_per_group or n_mut < min_cells_per_group:
            print(f"    Skipping group {group_value}: Not enough cells (< {min_cells_per_group}). Control: {n_ctrl}, Mutant: {n_mut}")
        ...
        min_cells_absolute = 3 # Define an absolute minimum
        min_percentage_of_cells = 0.10 # 10%
        
        calculated_min_cells_by_percentage = int(np.ceil(adata_subset.n_obs * min_percentage_of_cells))
        effective_min_cells = max(min_cells_absolute, calculated_min_cells_by_percentage)
        
        print(f"    Filtering genes: Requiring expression in >= {effective_min_cells} cells (max of {min_cells_absolute} absolute and 10% of {adata_subset.n_obs})...")
        ...
        if adata_subset.n_vars == 0:
            ...
            dge_results[group_value] = (None, n_ctrl, n_mut)
            skipped_dge.append({grouping_key: group_value, 'reason': f'No genes left after filtering (min_cells={min_cells_threshold}, 10%)'})
            continue
        ...
        try:
            ...
            if not result_df.empty:
                ...
                if not valid_genes:
                    ...
                    print(f"      Added mean expression columns.")
            # --- End mean expression ---
            ...
            file_base = f'dge_both_genomes_{group_value}_mut_vs_ctrl' # Use group_value in filename
            dge_csv_path = overall_dge_dir / f'{file_base}.csv' # Use pathlib Path object
            result_df.to_csv(os.path.normpath(str(dge_csv_path)), index=False) # Normalize path string
            print(f"    DGE results saved to {dge_csv_path}")

            # Save DGE results table (XLSX) - Use specific subdir
            if openpyxl:
                try:
                    dge_xlsx_path = overall_dge_dir / f'{file_base}.xlsx' # Use pathlib Path object
                    result_df.to_excel(os.path.normpath(str(dge_xlsx_path)), index=False, engine='openpyxl') # Normalize path string
            ...
            save_significant_degs(
                ...
                # Using default thresholds padj=0.05, logfc=1.0
            )
    ...
    if skipped_dge:
        ...
        skipped_output_path = overall_dge_dir / f'dge_overall_skipped_{grouping_key}.csv' # Use pathlib Path object
        skipped_df.to_csv(os.path.normpath(str(skipped_output_path)), index=False) # Normalize path string
...
def run_genotype_specific_dge(adata, grouping_key, dge_output_dir, plot_output_dir, layer=None):
    ...
    # Define and create specific output subdirectories
    # Use pathlib for robust path creation
    gs_dge_dir = pathlib.Path(dge_output_dir) / 'geno_spec_cond_comp'
    gs_plot_dir = pathlib.Path(plot_output_dir) / 'geno_spec_cond_comp'
    gs_dge_dir.mkdir(parents=True, exist_ok=True)
    ...
    if 'genotype_condition' not in adata.obs.columns:
        ...
        if not pd.api.types.is_categorical_dtype(adata.obs['genotype_condition']):
    ...
    for group_value in groups:
        ...
        if n_ctrl < min_cells_per_group or n_mut < min_cells_per_group:
            print(f"    Skipping {genotype} for group {group_value}: Not enough cells (< {min_cells_per_group}). {group_ctrl}: {n_ctrl}, {group_mut}: {n_mut}")
            ...
            min_cells_absolute_emx1 = 3
            min_percentage_of_cells_emx1 = 0.10
            
            calculated_min_cells_by_percentage_emx1 = int(np.ceil(adata_group_emx1.n_obs * min_percentage_of_cells_emx1))
            effective_min_cells_emx1 = max(min_cells_absolute_emx1, calculated_min_cells_by_percentage_emx1)

            print(f"    Filtering genes: Requiring expression in >= {effective_min_cells_emx1} cells (max of {min_cells_absolute_emx1} absolute and 10% of {adata_group_emx1.n_obs})...")
            ...
            if adata_group_emx1.n_vars == 0:
                ...
                dge_by_genotype[genotype][group_value] = (None, n_ctrl, n_mut)
                skipped_dge_genotype[genotype].append({grouping_key: group_value, 'reason': f'No genes left after filtering (min_cells={min_cells_threshold_emx1}, 10%)'})
                ...
                # Extract results for the specific comparison (Mutant vs Control)
                    ...
                    # print(f"    DEBUG ({genotype}, {group_value}): Columns after get_df: {list(result_df.columns)}") # Removed DEBUG

                    # --- Manually add reference PTS if missing ---
                    pts_col_g1 = 'pts' if 'pts' in result_df.columns else 'pct_nz_group' if 'pct_nz_group' in result_df.columns else None
                    pts_col_g2 = 'pts_rest' if 'pts_rest' in result_df.columns else 'pct_nz_reference' if 'pct_nz_reference' in result_df.columns else None

                    if pts_col_g1 and not pts_col_g2: # If group pts exists but reference pts is missing
                        # print(f"    DEBUG ({genotype}, {group_value}): Reference PTS column missing. Attempting to retrieve manually.") # Removed DEBUG
                        try:
                            # Access the full pts dataframe stored by rank_genes_groups
                            pts_df_full = adata_group_emx1.uns[key_added]['pts']
                            # Identify the reference group column name (should be 'Control' here)
                            ref_group_col = 'Control'
                            if ref_group_col in pts_df_full.columns:
                                # Map the reference pts values to result_df based on gene names
                                # Convert structured array names to simple list of strings
                                gene_names_list = list(adata_group_emx1.uns[key_added]['names'].flatten())
                                ref_pts_map = pts_df_full.set_index(pd.Index(gene_names_list))[ref_group_col]
                                result_df['pts_rest'] = result_df['names'].map(ref_pts_map)
                                # print(f"    DEBUG ({genotype}, {group_value}): Successfully added 'pts_rest' column.") # Removed DEBUG
                                # Ensure the group column is also named consistently if needed
                                if pts_col_g1 == 'pct_nz_group' and 'pts' not in result_df.columns:
                                     result_df.rename(columns={'pct_nz_group': 'pts'}, inplace=True)
                                     # print(f"    DEBUG ({genotype}, {group_value}): Renamed '{pts_col_g1}' to 'pts'.") # Removed DEBUG
                            else:
                                print(f"    WARNING ({genotype}, {group_value}): Reference group '{ref_group_col}' not found in adata.uns['{key_added}']['pts'].columns: {list(pts_df_full.columns)}")
                        except KeyError:
                            print(f"    WARNING ({genotype}, {group_value}): Could not find 'pts' key in adata.uns['{key_added}'].")
                        except Exception as e:
                            print(f"    ERROR ({genotype}, {group_value}): Error manually retrieving reference PTS: {e}")
                    ...
                    if not result_df.empty:
                        ...
                        if not valid_genes:
                            ...
                            if hasattr(data_matrix, "toarray"): data_matrix = data_matrix.toarray()
                    ...
                    # Save DGE results table (CSV) - Use specific subdir
                    file_base = f'dge_{genotype}_{group_value}_mut_vs_ctrl' # Use group_value in filename
                    dge_csv_path = gs_dge_dir / f'{file_base}.csv' # Use pathlib Path object
                    result_df.to_csv(os.path.normpath(str(dge_csv_path)), index=False) # Normalize path string
                    print(f"    DGE results saved to {dge_csv_path}")
                    ...
                    if openpyxl:
                        try:
                            dge_xlsx_path = gs_dge_dir / f'{file_base}.xlsx' # Use pathlib Path object
                            result_df.to_excel(os.path.normpath(str(dge_xlsx_path)), index=False, engine='openpyxl') # Normalize path string
                            print(f"    DGE results saved to {dge_xlsx_path}")
                        except Exception as e:
        ...
        if n_ctrl < min_cells_per_group or n_mut < min_cells_per_group:
            ...
            min_cells_absolute_nes = 3
            min_percentage_of_cells_nes = 0.10

            calculated_min_cells_by_percentage_nes = int(np.ceil(adata_group_nes.n_obs * min_percentage_of_cells_nes))
            effective_min_cells_nes = max(min_cells_absolute_nes, calculated_min_cells_by_percentage_nes)
            
            print(f"    Filtering genes: Requiring expression in >= {effective_min_cells_nes} cells (max of {min_cells_absolute_nes} absolute and 10% of {adata_group_nes.n_obs})...")
            ...
            if adata_group_nes.n_vars == 0:
                print(f"    Skipping {genotype} for group {group_value}: No genes remaining after filtering.")
                dge_by_genotype[genotype][group_value] = (None, n_ctrl, n_mut)
                skipped_dge_genotype[genotype].append({grouping_key: group_value, 'reason': f'No genes left after filtering (min_cells={min_cells_threshold_nes}, 10%)'})
                ...
                # Extract results for the specific comparison (Mutant vs Control)
                    ...
                    if pts_col_g1 and not pts_col_g2: # If group pts exists but reference pts is missing
                        ...
                        try:
                            # Access the full pts dataframe stored by rank_genes_groups
                            pts_df_full = adata_group_nes.uns[key_added]['pts']
                            # Identify the reference group column name (should be 'Control' here)
                            ref_group_col = 'Control'
                            if ref_group_col in pts_df_full.columns:
                                # Map the reference pts values to result_df based on gene names
                                # Convert structured array names to simple list of strings
                                gene_names_list = list(adata_group_nes.uns[key_added]['names'].flatten())
                                ref_pts_map = pts_df_full.set_index(pd.Index(gene_names_list))[ref_group_col]
                                result_df['pts_rest'] = result_df['names'].map(ref_pts_map)
                                # print(f"    DEBUG ({genotype}, {group_value}): Successfully added 'pts_rest' column.") # Removed DEBUG
                                # Ensure the group column is also named consistently if needed
                    ...
                    file_base = f'dge_{genotype}_{group_value}_mut_vs_ctrl' # Use group_value in filename
                    dge_csv_path = gs_dge_dir / f'{file_base}.csv' # Use pathlib Path object
                    result_df.to_csv(str(dge_csv_path), index=False) # Convert Path to string for to_csv
                    print(f"    DGE results saved to {dge_csv_path}")
                    ...
                    if openpyxl:
                        try:
                            dge_xlsx_path = gs_dge_dir / f'{file_base}.xlsx' # Use pathlib Path object
                            result_df.to_excel(str(dge_xlsx_path), index=False, engine='openpyxl') # Convert Path to string for to_excel
                            print(f"    DGE results saved to {dge_xlsx_path}")
                        except Exception as e:
                            print(f"    Error saving DGE results to Excel for {genotype}, group {group_value}: {e}")
                    else:
    ...
    for genotype, skipped_list in skipped_dge_genotype.items():
        if skipped_list:
            ...
            skipped_output_path = gs_dge_dir / f'dge_{genotype}_skipped_{grouping_key}.csv' # Use pathlib Path object
            skipped_df.to_csv(str(skipped_output_path), index=False) # Convert Path to string for to_csv
...
def run_genotype_comparison_dge(adata, grouping_key, dge_output_dir, plot_output_dir, layer=None):
    ...
    # Define and create specific output subdirectories
    # Use pathlib for robust path creation
    gc_dge_dir = pathlib.Path(dge_output_dir) / 'condition_specific_genotypes_comparison'
    gc_plot_dir = pathlib.Path(plot_output_dir) / 'condition_specific_genotypes_comparison'
    gc_dge_dir.mkdir(parents=True, exist_ok=True)
    gc_plot_dir.mkdir(parents=True, exist_ok=True)
    ...
    for group_value in groups:
        ...
        if n_ref < min_cells_per_group or n_comp < min_cells_per_group:
            print(f"    Skipping {condition} for group {group_value}: Not enough cells (< {min_cells_per_group}). {group_ref}: {n_ref}, {group_comp}: {n_comp}")
            ...
            min_cells_absolute_ctrl = 3
            min_percentage_of_cells_ctrl = 0.10

            calculated_min_cells_by_percentage_ctrl = int(np.ceil(adata_group_ctrl.n_obs * min_percentage_of_cells_ctrl))
            effective_min_cells_ctrl = max(min_cells_absolute_ctrl, calculated_min_cells_by_percentage_ctrl)

            print(f"    Filtering genes for {condition}: Requiring expression in >= {effective_min_cells_ctrl} cells (max of {min_cells_absolute_ctrl} absolute and 10% of {adata_group_ctrl.n_obs})...")
            ...
            if adata_group_ctrl.n_vars == 0:
                ...
                dge_genotype_within_condition[condition][group_value] = (None, n_ref, n_comp)
                skipped_dge_within_condition[condition].append({grouping_key: group_value, 'reason': f'No genes left after filtering (min_cells={min_cells_threshold_ctrl}, 10%)'})
                ...
                try:
                    ...
                    print(f"    DEBUG ({condition}, {group_value}): Columns after get_df: {list(result_df.columns)}") # DEBUG
                    ...
                    if pts_col_g1 and not pts_col_g2: # If group pts exists but reference pts is missing
                        print(f"    DEBUG ({condition}, {group_value}): Reference PTS column missing. Attempting to retrieve manually.")
                        try:
                            # Access the full pts dataframe stored by rank_genes_groups
                            pts_df_full = adata_group_ctrl.uns[key_added_ctrl]['pts']
                            # Identify the reference group column name (should be ref_genotype, e.g., 'Emx1')
                            ref_group_col = ref_genotype # Defined earlier in the function
                            if ref_group_col in pts_df_full.columns:
                                # Map the reference pts values to result_df based on gene names
                                ref_pts_map = pts_df_full.set_index(adata_group_ctrl.uns[key_added_ctrl]['names'].astype(str))[ref_group_col]
                                result_df['pts_rest'] = result_df['names'].map(ref_pts_map)
                                print(f"    DEBUG ({condition}, {group_value}): Successfully added 'pts_rest' column.")
                                # Ensure the group column is also named consistently if needed
                                if pts_col_g1 == 'pct_nz_group' and 'pts' not in result_df.columns:
                                     result_df.rename(columns={'pct_nz_group': 'pts'}, inplace=True)
                                     print(f"    DEBUG ({condition}, {group_value}): Renamed '{pts_col_g1}' to 'pts'.")
                            else:
                                print(f"    DEBUG WARNING ({condition}, {group_value}): Reference group '{ref_group_col}' not found in adata.uns['{key_added_ctrl}']['pts'].columns: {list(pts_df_full.columns)}")
                        except KeyError:
                            print(f"    DEBUG WARNING ({condition}, {group_value}): Could not find 'pts' key in adata.uns['{key_added_ctrl}'].")
                        except Exception as e:
                            print(f"    DEBUG ERROR ({condition}, {group_value}): Error manually retrieving reference PTS: {e}")
                    ...
                    # Save DGE results table (CSV) - Use specific subdir
                    file_base = f'dge_{condition}_cond_{group_value}_{comp_genotype}_vs_{ref_genotype}' # Use group_value in filename
                    dge_csv_path = gc_dge_dir / f'{file_base}.csv' # Use pathlib Path object
                    result_df.to_csv(os.path.normpath(str(dge_csv_path)), index=False) # Normalize path string
                    print(f"    DGE results saved to {dge_csv_path}")
                    ...
                    if openpyxl:
                        try:
                            dge_xlsx_path = gc_dge_dir / f'{file_base}.xlsx' # Use pathlib Path object
                            result_df.to_excel(os.path.normpath(str(dge_xlsx_path)), index=False, engine='openpyxl') # Normalize path string
                            print(f"    DGE results saved to {dge_xlsx_path}")
                        except Exception as e:
        ...
        if n_ref < min_cells_per_group or n_comp < min_cells_per_group:
            ...
            min_cells_absolute_mut = 3
            min_percentage_of_cells_mut = 0.10

            calculated_min_cells_by_percentage_mut = int(np.ceil(adata_group_mut.n_obs * min_percentage_of_cells_mut))
            effective_min_cells_mut = max(min_cells_absolute_mut, calculated_min_cells_by_percentage_mut)

            print(f"    Filtering genes for {condition}: Requiring expression in >= {effective_min_cells_mut} cells (max of {min_cells_absolute_mut} absolute and 10% of {adata_group_mut.n_obs})...")
            ...
            if adata_group_mut.n_vars == 0:
                print(f"    Skipping {condition} for group {group_value}: No genes remaining after filtering.")
                dge_genotype_within_condition[condition][group_value] = (None, n_ref, n_comp)
                skipped_dge_within_condition[condition].append({grouping_key: group_value, 'reason': f'No genes left after filtering (min_cells={min_cells_threshold_mut}, 10%)'})
                ...
                try:
                    ...
                    if pts_col_g1 and not pts_col_g2: # If group pts exists but reference pts is missing
                        ...
                        try:
                            # Access the full pts dataframe stored by rank_genes_groups
                            pts_df_full = adata_group_mut.uns[key_added_mut]['pts']
                            # Identify the reference group column name (should be ref_genotype, e.g., 'Emx1')
                            ref_group_col = ref_genotype # Defined earlier in the function
                            if ref_group_col in pts_df_full.columns:
                                # Map the reference pts values to result_df based on gene names
                                ref_pts_map = pts_df_full.set_index(adata_group_mut.uns[key_added_mut]['names'].astype(str))[ref_group_col]
                                result_df['pts_rest'] = result_df['names'].map(ref_pts_map)
                                print(f"    DEBUG ({condition}, {group_value}): Successfully added 'pts_rest' column.")
                                # Ensure the group column is also named consistently if needed
                                if pts_col_g1 == 'pct_nz_group' and 'pts' not in result_df.columns:
                                ...
                                print(f"    DEBUG WARNING ({condition}, {group_value}): Reference group '{ref_group_col}' not found in adata.uns['{key_added_mut}']['pts'].columns: {list(pts_df_full.columns)}")
                        except KeyError:
                            print(f"    DEBUG WARNING ({condition}, {group_value}): Could not find 'pts' key in adata.uns['{key_added_mut}'].")
                        except Exception as e:
                            print(f"    DEBUG ERROR ({condition}, {group_value}): Error manually retrieving reference PTS: {e}")
    ...
    for condition, skipped_list in skipped_dge_within_condition.items():
        if skipped_list:
            skipped_df = pd.DataFrame(skipped_list)
            skipped_output_path = gc_dge_dir / f'dge_{condition}_cond_comparison_skipped_{grouping_key}.csv' # Use pathlib Path object
            skipped_df.to_csv(str(skipped_output_path), index=False) # Convert Path to string for to_csv
...
def run_cluster_comparison_dge(adata, grouping_key, dge_output_dir, plot_output_dir, layer=None, method='t-test'):
    ...
    # Use pathlib for robust path creation
    # The dge_output_dir passed from raw_16 is already the target 'biomarkers' directory
    cluster_dge_dir = pathlib.Path(dge_output_dir) # Convert to Path object
    cluster_plot_dir = pathlib.Path(plot_output_dir) # Convert to Path object
    cluster_dge_dir.mkdir(parents=True, exist_ok=True)
    ...
    print(f"  Filtering genes globally: Requiring expression in >= {min_cells_global} cells...")
    sc.pp.filter_genes(adata_filt, min_cells=min_cells_global)
    print(f"  Genes after global filtering: {adata_filt.n_vars}")
    if adata_filt.n_vars == 0:
        print(f"  Skipping cluster comparison DGE: No genes remaining after global filtering (min_cells={min_cells_global}).")
        return {}
    ...
    try:
        if layer is None:
             print(f"    WARNING: Running cluster marker DGE on adata.X. Ensure this data representation is appropriate (see function docstring warning).")
        sc.tl.rank_genes_groups(
    ...
    for group_value in groups:
        ...
        try:
            ...
            file_base_top50 = f'dge_cluster_comparison_{group_value}_vs_Rest_top50' # Filename for top 50

            # Save Top 50 DGE results table (CSV)
            dge_csv_path_top50 = cluster_dge_dir / f'{file_base_top50}.csv' # Use pathlib Path object
            top_50_df.to_csv(str(dge_csv_path_top50), index=False) # Convert Path to string for to_csv
            print(f"    Top 50 Cluster marker results saved to {dge_csv_path_top50}")

            # Save Top 50 DGE results table (XLSX)
            if openpyxl:
                try:
                    dge_xlsx_path_top50 = cluster_dge_dir / f'{file_base_top50}.xlsx' # Use pathlib Path object
                    top_50_df.to_excel(str(dge_xlsx_path_top50), index=False, engine='openpyxl') # Convert Path to string for to_excel
            ...
            save_significant_degs(
                ...
                # LogFC threshold might need adjustment for marker identification
                # Using default padj=0.05, logfc=1.0 for now
            )
            # --- End save significant DEG lists ---
    ...
    if skipped_clusters:
        ...
        skipped_output_path = cluster_dge_dir / f'dge_cluster_comparison_skipped_{grouping_key}.csv' # Use pathlib Path object
        skipped_df.to_csv(str(skipped_output_path), index=False) # Convert Path to string for to_csv
    ...
    if rank_genes_key in adata_filt.uns and groups: # Check if results exist and groups were found
        ...
        try:
            ...
            # Convert Path object to string for scanpy setting
            sc.settings.figdir = str(cluster_plot_dir)
            ...
            # Construct expected path using pathlib for consistency in the print statement
            expected_plot_path = cluster_plot_dir / f"rank_genes_groups_{plot_filename_suffix[1:]}" # Remove leading '_'
            sc.pl.rank_genes_groups(adata_filt, n_genes=25, sharey=False, key=rank_genes_key, save=plot_filename_suffix, show=False)
            print(f"  Marker plot saved to {expected_plot_path}")
        except Exception as e:

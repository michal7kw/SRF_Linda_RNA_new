...
print(data.obs["cell_type_L1"].unique())
...
# %%
unique_cell_type = data.obs["cell_type_L1"].unique()
cmap = plt.get_cmap("tab10")
cell_type_color_dict = {cell_type: cmap(i % cmap.N) for i, cell_type in enumerate(unique_cell_type)}
cell_type_colors = [cell_type_color_dict[cell_type] for cell_type in data.obs["cell_type_L1"]]

# %%
plt_legend_cell_type = exvis.get_legend_patch(data.obs["cell_type_L1"], cell_type_colors )
...
vis.plot_pca(pca_scores, NUM_COMP_TO_VIS, 
               cell_color_vec= cell_type_colors, 
               legend_handles=True,
               title=title,
...
# %%
fcat_cell_type = efca.FCAT(data.obs["cell_type_L1"], factor_scores, scale='standard', mean='arithmatic')

# %%
### concatenate FCAT table for protocol and cell line
fcat = pd.concat([fcat_condition, fcat_cell_type], axis=0)
...
asv_cell_line = met.average_scaled_var(factor_scores, covariate_vector=data.obs["cell_type_L1"], mean_type='arithmetic')
...
svt_cell_line = met.scaled_var_table(factor_scores, data.obs["cell_type_L1"])
...
svt = pd.concat([svt_cell_line, svt_condition, svt_genotype], axis=0)

...
LOG_DIR="${SCRIPT_DIR}/../logs/enrichr_runs_$(date +%Y%m%d_%H%M%S)"
...
echo "Running Enrichr analysis scripts in parallel. Logs will be saved to ${LOG_DIR}"
...
run_script() {
    ...
    if [ -n "$cluster_name" ] && [ -n "$cluster_column" ]; then
        ...
        echo "  - Starting enrichr_conditions.py for genotype=${genotype}, cluster=${cluster_name}"
        python enrichr_conditions.py --genotype "${genotype}" --merge_clusters "False" --cluster_name "${cluster_name}" --cluster_column "${cluster_column}" > "${LOG_DIR}/enrichr_conditions_${log_suffix}.log" 2>&1 &
    else
        local log_suffix="${genotype}_merge_${merge_clusters}"
        echo "  - Starting enrichr_conditions.py for genotype=${genotype}, merge_clusters=${merge_clusters}"
        python enrichr_conditions.py --genotype "${genotype}" --merge_clusters "${merge_clusters}" > "${LOG_DIR}/enrichr_conditions_${log_suffix}.log" 2>&1 &
    fi
}
...
# echo "Starting Enrichr analysis for GABA cluster..."
...
echo "All Enrichr analysis scripts have completed."
...
run_script() {
    ...
    echo "  - Starting gsea_clusters_unified.py for condition=${condition}, genotype=${genotype}, merge_clusters=${merge_clusters}"
    python gsea_clusters_unified.py --condition "${condition}" --genotype "${genotype}" --merge_clusters "${merge_clusters}" > "${LOG_DIR}/gsea_clusters_${log_suffix}.log" 2>&1 &
}

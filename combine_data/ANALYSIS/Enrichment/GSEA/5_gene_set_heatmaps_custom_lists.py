#!/usr/bin/env python3
"""
Gene Set Scoring with Custom Gene Lists - Handling Positive/Negative Correlations

This script performs gene set scoring using custom gene lists where each gene set
has separate lists for positively and negatively correlated genes. Creates three
types of visualizations:
1. Positive scores only
2. Negative scores only
...
Output:
    - Publication-ready heatmaps for each scoring type
...
GENE_LISTS_PATH = os.path.join(WORKING_DIR, "ANALYSIS/Enrichment/GSEA/GENE_LIST/gene_lists.csv")

# Output directory
OUTPUT_DIR = os.path.join(BASE_RESULTS_DIR, "gene_set_enrichment_heatmaps_custom")
...
def create_individual_geneset_heatmaps(adata, gene_sets_dict, cell_type_column,
    ...
    for gene_set_name in gene_sets_dict.keys():
        ...
        base_filename = f'{safe_gene_set}_{score_type}_{cell_type_column}_{split_mode}'
...
def create_summary_report(adata, gene_sets_dict, score_type, output_dir):
    ...
    summary_df.to_csv(os.path.join(output_dir, f'gene_set_scores_summary_{score_type}.csv'),
                     index=False)

    print(f"  Saved: gene_set_scores_summary_{score_type}.csv")
...
def main():
    ...
    print("=" * 70)
    print("Gene Set Scoring with Custom Gene Lists (Up/Down Regulation)")
    print("=" * 70)
    ...
    for score_type in score_types:
        ...
        adata_scored = calculate_gene_set_scores_with_direction(
            adata, gene_sets_dict, score_type=score_type
        )

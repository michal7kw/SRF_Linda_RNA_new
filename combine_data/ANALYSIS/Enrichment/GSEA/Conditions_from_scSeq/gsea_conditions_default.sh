...
LOG_DIR="${SCRIPT_DIR}/../logs/gsea_runs_default_$(date +%Y%m%d_%H%M%S)"
...
echo "Running GSEA analysis scripts (default protocol) in parallel. Logs will be saved to ${LOG_DIR}"
...
run_script() {
    ...
    if [ -n "$cluster_name" ] && [ -n "$cluster_column" ]; then
        ...
        echo "  - Starting gsea_conditions_gsea_default.py for genotype=${genotype}, cluster=${cluster_name}"
        python gsea_conditions_gsea_default.py --genotype "${genotype}" --merge_clusters "False" --cluster_name "${cluster_name}" --cluster_column "${cluster_column}" > "${LOG_DIR}/gsea_conditions_${log_suffix}.log" 2>&1 &
    else
        local log_suffix="${genotype}_merge_${merge_clusters}"
        echo "  - Starting gsea_conditions_gsea_default.py for genotype=${genotype}, merge_clusters=${merge_clusters}"
        python gsea_conditions_gsea_default.py --genotype "${genotype}" --merge_clusters "${merge_clusters}" > "${LOG_DIR}/gsea_conditions_${log_suffix}.log" 2>&1 &
    fi
}

...
run_script() {
    ...
    if [ -n "$cluster_name" ] && [ -n "$cluster_column" ]; then
        ...
        echo "  - Starting gsea_conditions_filter_unified.py for genotype=${genotype}, focus=${focus}, cluster=${cluster_name}"
    else
        log_suffix+="_merge_${merge_clusters}"
        echo "  - Starting gsea_conditions_filter_unified.py for genotype=${genotype}, focus=${focus}, merge_clusters=${merge_clusters}"
    fi

    # Use eval to correctly handle parameters with spaces
    eval "python gsea_conditions_filter_unified.py ${params}" > "${LOG_DIR}/gsea_conditions_filter_${log_suffix}.log" 2>&1 &
}

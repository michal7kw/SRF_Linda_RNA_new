...
Features:
- Plots only terms from selected_terms.txt
...
OUTPUT_DIR = os.path.join(BASE_RESULTS_DIR, 'gsea_selected_terms')
...
def normalize_term_name(term):
    ...
    prefixes = ['GOBP_', 'GOCC_', 'GOMF_', 'GO_', 'REACTOME_', 'R-HSA-']
...
def load_gsea_results(genotype_dir, cluster_name):
    """Load GSEA results for a given genotype and cluster."""
    comparison_name = "Mutant_vs_Control"

    # Try multiple possible file patterns
    possible_files = [
        pathlib.Path(GSEA_BASE_DIR) / genotype_dir / f"{cluster_name}_{comparison_name}" / "GSEA_Full_Report_All_Genesets.csv",
        pathlib.Path(GSEA_BASE_DIR) / genotype_dir / f"{cluster_name}_{comparison_name}" / "GSEA_Full_Report_All_Genesets.xlsx",
        pathlib.Path(GSEA_BASE_DIR) / genotype_dir / f"{cluster_name}_{comparison_name}" / "GSEA_Summary_All_Genesets.csv",
        pathlib.Path(GSEA_BASE_DIR) / genotype_dir / f"{cluster_name}_{comparison_name}" / "GSEA_Summary_All_Genesets.xlsx",
        pathlib.Path(GSEA_BASE_DIR) / genotype_dir / f"{cluster_name}_{comparison_name}" / "GSEA_results.csv",
        pathlib.Path(GSEA_BASE_DIR) / genotype_dir / f"{cluster_name}_{comparison_name}" / "GSEA_results.xlsx",
    ]
    ...
    if file_path is None:
        print(f"Warning: No GSEA results file found for {genotype_dir}/{cluster_name}")
...
def create_selected_terms_heatmap(cluster_name, selected_terms_file=SELECTED_TERMS_FILE,
                                   reference_cluster=None, reference_order=None, reference_vmax=None):
    """
    ...
    reference_cluster : str, optional
        If provided, load data from this cluster to determine term order and color scale
    reference_order : list, optional
        Pre-computed term order to use (if reference_cluster is not provided)
    reference_vmax : float, optional
        Pre-computed color scale maximum (if reference_cluster is not provided)

    Returns:
    --------
    tuple : (term_order, vmax) - for use in subsequent plots
    """
    print(f"\n{'='*70}")
    print(f"Creating heatmap for selected terms in {cluster_name}")
    if reference_cluster:
        print(f"Using term order and color scale from: {reference_cluster}")
    print(f"{'='*70}")

    # Load selected terms
    selected_terms = load_selected_terms(selected_terms_file)

    # If reference cluster is specified, load it first to get order and scale
    if reference_cluster and reference_cluster != cluster_name:
        print(f"\n--- Loading reference cluster: {reference_cluster} ---")
        ref_results = []

        for genotype_name, genotype_dir in GENOTYPES.items():
            df = load_gsea_results(genotype_dir, reference_cluster)
            if df is not None:
                print(f"\nMatching selected terms in {genotype_name} data (reference):")
                ...
                if len(df_matched) > 0:
                    ...
                    ref_results.append(df_renamed)

        if len(ref_results) == len(GENOTYPES):
            ref_merged = pd.concat(ref_results, axis=1, join='outer')
            ref_nes_cols = [f"{name}_nes" for name in GENOTYPES.keys()]

            # Get term order from reference
            ref_merged['mean_abs_nes'] = ref_merged[ref_nes_cols].abs().mean(axis=1)
            ref_merged = ref_merged.sort_values('mean_abs_nes', ascending=False)
            reference_order = list(ref_merged.index)

            # Get color scale from reference
            reference_vmax = np.nanmax(np.abs(ref_merged[ref_nes_cols].values))
            reference_vmax = max(reference_vmax, 0.1)

            print(f"\n✓ Reference term order: {len(reference_order)} terms")
            print(f"✓ Reference color scale: ±{reference_vmax:.2f}")
        else:
            print(f"\nWarning: Could not load reference cluster {reference_cluster}")
            reference_order = None
            reference_vmax = None

    # Load data for target cluster
    print(f"\n--- Loading target cluster: {cluster_name} ---")
    all_results = []

    for genotype_name, genotype_dir in GENOTYPES.items():
        df = load_gsea_results(genotype_dir, cluster_name)
        if df is not None:
    ...
    # Sort by mean absolute NES to order terms (or use reference order)
    if reference_order is not None:
        # Use reference order, but only for terms that exist in current data
        existing_terms = set(merged_df.index)
        ordered_terms = [t for t in reference_order if t in existing_terms]

        # Add any new terms not in reference (sorted by their own mean_abs_nes)
        new_terms = existing_terms - set(reference_order)
        if new_terms:
            print(f"\nNote: {len(new_terms)} terms not in reference cluster, appending at end")
            merged_df['mean_abs_nes'] = merged_df[nes_cols].abs().mean(axis=1)
            new_terms_sorted = merged_df.loc[list(new_terms)].sort_values('mean_abs_nes', ascending=False).index.tolist()
            ordered_terms.extend(new_terms_sorted)

        merged_df = merged_df.loc[ordered_terms]
        print(f"✓ Applied reference term order ({len(ordered_terms)} terms)")
    else:
        # Default: sort by mean absolute NES
        merged_df['mean_abs_nes'] = merged_df[nes_cols].abs().mean(axis=1)
    ...
    # Color scale (use reference if provided)
    if reference_vmax is not None:
        vmax = reference_vmax
        print(f"✓ Using reference color scale: ±{vmax:.2f}")
    else:
    ...
    # Save outputs
    base_filename = f"GSEA_Selected_Terms_{cluster_name}"
    ...
    # Return order and scale for potential reuse
    return list(merged_df.index), vmax


def main():
    """Main function to generate selected terms GSEA heatmap."""
    parser = argparse.ArgumentParser(
        description="Generate GSEA heatmap with only selected terms (no FDR indicators)."
    )
    parser.add_argument(
        '--cluster',
        type=str,
        default='Combined_GC',
        help="Cluster name to analyze (options: Combined_GC, Mature_GC, GABA)"
    )
    parser.add_argument(
        '--terms_file',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Reference cluster to use for term order and color scale (e.g., GABA)"
    )

    args = parser.parse_args()

    print("=" * 70)
    print("Selected Terms GSEA Heatmap Generator")
    print("=" * 70)
    print(f"Output directory: {OUTPUT_DIR}")
    print(f"Cluster: {args.cluster}")
    print(f"Selected terms file: {args.terms_file}")
    if args.reference_cluster:
        print(f"Reference cluster: {args.reference_cluster}")

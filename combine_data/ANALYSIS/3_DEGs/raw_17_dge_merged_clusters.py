...
# MODIFIED: Changed output directory for merged cluster analysis
PARENT_OUTPUT_DIR = os.path.join(INPUT_DIR, "DEGs_merged_clusters")

# CUSTOM_ANALYSIS =  None
CUSTOM_ANALYSIS =  "FC_0_25"

# This is the original grouping key, used for mapping to merged clusters
DEG_BY_ORIGINAL = "cell_type_L2"
...
print(f"Original {DEG_BY_ORIGINAL} clusters:")
print(adata.obs[DEG_BY_ORIGINAL].value_counts())

# Reset to original settings
pd.set_option('display.max_rows', original_max_rows)

# %% [markdown]
# # Merge Clusters for DGE analysis

# %%
# Define the mapping for merging clusters
# adata.obs[DEG_BY_ORIGINAL].unique()
# ['Astrocytes', 'Unknown', 'CA3', 'ENT', 'PPP', 'CA1', 'MGE-derived', 'Mature GC', 'MOL', 
# 'ProS', 'Microglia', 'Subiculum', 'CGE-derived', 'VIP', 'CA2', 'Sncg', 'OPC', 'ABC', 
# 'NFOL', 'Lamp5', 'SST', 'PV', 'Immature GC', 'VLMC', 'Endothelial', 'Lamp5 (DG)', 
# 'nIPC', 'Pericytes', 'Car3', 'VIP (DG)', 'Mossy cells', 'Neuroblast', 'Cajal-Retzius']
mge_clusters = ['MGE-derived', 'PV', 'SST']
cge_clusters = ['CGE-derived', 'Lamp5', 'Lamp5 (DG)', 'VIP', 'VIP (DG)', 'Sncg']

# Create a new metadata column for the merged clusters
adata.obs['merged_clusters'] = 'Other'
adata.obs.loc[adata.obs[DEG_BY_ORIGINAL].isin(mge_clusters), 'merged_clusters'] = 'MGE-derived INS (merge)'
adata.obs.loc[adata.obs[DEG_BY_ORIGINAL].isin(cge_clusters), 'merged_clusters'] = 'CGE-derived INS (merge)'

# Convert to categorical
adata.obs['merged_clusters'] = adata.obs['merged_clusters'].astype('category')

# Filter out cells that are not part of the merged clusters
adata = adata[adata.obs['merged_clusters'] != 'Other'].copy()

# Check the new merged clusters
print("\nValue counts for merged_clusters:")
print(adata.obs['merged_clusters'].value_counts())

# Set the new grouping key for DGE analysis
DEG_BY = "merged_clusters"

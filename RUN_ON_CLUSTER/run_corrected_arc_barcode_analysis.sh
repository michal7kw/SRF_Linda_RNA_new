...
#
# Is is the same as run_corrected_arc_barcode_analysis_vectorized.sh
...
if [[ -z "$ARC_ATAC_LIST" || -z "$ARC_GEX_LIST" ]]; then
    ...
    BARCODE_DOWNLOAD_DIR="/beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda_top/SRF_Linda_RNA/arc_barcode_lists"
...
for (rna_sample in names(sample_mapping)) {
  ...
  # Apply ARC translation (CORRECTED METHOD - VECTORIZED)
  message("  Applying ARC ATAC→GEX translation...")
  start_time <- Sys.time()
  message("  Translation started at: ", start_time)

  total_atac_barcodes <- length(atac_16mers_clean)
  message("  Processing ", total_atac_barcodes, " ATAC barcodes using vectorized approach...")

  # Vectorized translation - much faster than for loop
  message("  Finding matches in ARC translation map...")
  matches <- atac_16mers_clean %in% names(atac_to_gex_map)
  translated_count <- sum(matches)

  message("  Found ", translated_count, " matches (", round(translated_count/total_atac_barcodes*100, 1), "%) - performing translation...")

  # Extract only the matching barcodes and translate them
  matching_atac_barcodes <- atac_16mers_clean[matches]
  translated_barcodes <- atac_to_gex_map[matching_atac_barcodes]

  # Get unique translated barcodes
  atac_barcodes_translated[[rna_sample]] <- unique(translated_barcodes)

  end_time <- Sys.time()
  translation_duration <- as.numeric(difftime(end_time, start_time, units = "mins"))
  message("  Translation completed at: ", end_time)
  message("  Translation duration: ", round(translation_duration, 2), " minutes")

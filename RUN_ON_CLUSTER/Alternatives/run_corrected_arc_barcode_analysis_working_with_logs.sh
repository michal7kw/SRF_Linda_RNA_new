...
for (rna_sample in names(sample_mapping)) {
  ...
  progress_interval <- max(100, floor(total_atac_barcodes / 1000))  # Report every 0.1% or 100, whichever is larger

  message("  Processing ", total_atac_barcodes, " ATAC barcodes (progress every ", progress_interval, " barcodes)...")

  for (i in seq_along(atac_16mers_clean)) {
    atac_16mer <- atac_16mers_clean[i]
    ...
    # Progress monitoring
    if (i %% progress_interval == 0 || i == total_atac_barcodes) {
      percent_complete <- round((i / total_atac_barcodes) * 100, 1)
      message("    Progress: ", i, "/", total_atac_barcodes, " (", percent_complete, "%) - ", translated_count, " matches so far")
    }
  }
